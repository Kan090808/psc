...
class Newspaper_X_Breadcrumbs
{
  ...
  public function __construct()
  {
    ...
    $defaults = array(
      'home_prefix' => get_theme_mod('newspaper_x_blog_breadcrumb_menu_prefix', esc_html__('', 'newspaper-x')),
      ...
      'tag_archive_prefix' => esc_html__('Tag:', 'newspaper-x'),
      'search_prefix' => esc_html__('Search:', 'newspaper-x'),
      'error_prefix' => esc_html__('404 - Page not Found', 'newspaper-x'),
    );

		// Setup a filter for changeable variables and merge it with the defaults
    $args = apply_filters('newspaper_x_breadcrumbs_defaults', $defaults);
  ...
  private function prepare_breadcrumb_html()
  {
    ...
    if (class_exists('bbPress') &&
      is_bbpress() && (bbp_is_topic_archive() || bbp_is_single_user() || bbp_is_search())) {
  ...
  {
    $this->html_markup = sprintf('<div class="newspaper-x-breadcrumbs">%s</div>', $this->html_markup);
  }
  ...
  private function get_breadcrumb_prefix()
  {
    ...
    if (!is_front_page()) {
      ...
      if ($this->home_prefix) {
        $prefix = sprintf('<span class="newspaper-x-breadcrumb-prefix">%s:</span>', esc_html($this->home_prefix));
      }
    }
  ...
  private function get_single_breadcrumb_markup($title, $link = '', $separator = true, $microdata = true)
  {
    ...
    if ($separator) {
      $separator_markup = sprintf('<span class="newspaper-x-breadcrumb-sep">%s</span>', $this->separator);
    }
